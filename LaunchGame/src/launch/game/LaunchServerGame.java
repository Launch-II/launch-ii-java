...
public class LaunchServerGame extends LaunchGame implements LaunchServerGameInterface
{
    ...
    protected void GameTick(int lMS)
    {
        ...
        for(OreMine oreMine : OreMines.values())
        {
            ...
            if(oreMine.GetGenerateOre())
            {
                ...
                int lValue = (int)(((.1 + random.nextFloat() * (1 - .1)) * (float)GetMaxPotentialOreMineReturn(oreMine)) + 0.5f);
    ...
    {   
            if(structure.GetChargeOwner())
            {
                ...
                if(owner != null)
                {
                    //Added the following "if" argument to charge players 10% (or $5) of what they pay when the structure is online.
                    if(!structure.GetRunning())
                    {
                        short OfflineUpkeep = (short) (lMaintenanceCost *0.1);
                        if(owner.SubtractWealth(5))
                        {
                        //Player was charged successfully.
                            structure.SetChargeOwnerTime(CHARGE_INTERVAL);
                            LaunchLog.Log(LaunchLog.LogType.GAME, LOG_NAME, String.format("%s charged for offline %s maintenance.", owner.GetName(), structure.GetTypeName()));
                        
                            if(bOffensiveCost)
                            Scoring_OffenceSpending(owner, 5);
                        
                            if(bDefensiveCost)
                            Scoring_DefenceSpending(owner, 5);
                        }
                        else
                        {
                        //Insufficient funds for offline upkeep. Inflict damage equal to 15% of structure's max health. -Corbin
                        short NeglectDamage = (short) (0.15 * structure.GetMaxHP());
                        structure.InflictDamage(NeglectDamage);
                        CreateReport(owner, new LaunchReport(String.format("Your %s took damage due to lack of upkeep. Sad!", structure.GetTypeName()), true, owner.GetID()));
                        }
                    }
                    else 
                    {   
                        ...
                        CreateEvent(new LaunchEvent(String.format("%s's %s went offline due to lack of funds. Sad!", owner.GetName(), structure.GetTypeName())));
                        CreateReport(owner, new LaunchReport(String.format("Your %s went offline due to lack of funds. Sad!", structure.GetTypeName()), true, owner.GetID()));
                        }
                    }
                }
                else
    ...
    public boolean ConstructOreMine(int lPlayerID)
    {
        ...
        {                                                                                                               //Made Ore ine health equal to 1/2 of StructureBaseHP. TODO: Add OreMineHealth to Config. -Corbin
            OreMine oreMine = new OreMine(GetAtomicID(lOreMineIndex, OreMines), player.GetPosition().GetCopy(), (short) (0.5 * config.GetStructureBaseHP()), (short) (0.5 * config.GetStructureBaseHP()), player.GetID(), player.GetRespawnProtected(), config.GetStructureBootTime(), CHARGE_INTERVAL, config.GetOreMineGenerateTime());
            AddOreMine(oreMine);
    ...
    public boolean MissileSlotUpgrade(int lPlayerID, int lMissileSiteID)
    {
        ...
        if(!missileSite.CanTakeNukes() && player.Functioning() && ((int)missileSystem.GetSlotCount() + (int)config.GetMissileUpgradeCount()) <= Defs.MAX_MISSILE_SLOTS)
        {
    ...
    public boolean MissileReloadUpgrade(int lPlayerID, int lMissileSiteID)
    {
        ...
        if(!missileSite.CanTakeNukes() && missileSite.GetOwnerID() == lPlayerID && player.Functioning())
        {
